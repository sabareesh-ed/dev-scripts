...
const swiper4 = new Swiper(".swiper4", {
  ...
  on: {
    init: function () {
      startProgressBar();
      animateContent();
      updatePagination(); // Update pagination for initial active slide
    },
...
// Function to update pagination and apply styles to the active slide
function updatePagination() {
  ...
  bullets.forEach((bullet) => {
    ...
    bullet.addEventListener("mouseenter", () => {
      svgPath.style.fill = "rgba(var(--swatch--brand), 0.6)"; // Use brand color with less opacity
    });

    bullet.addEventListener("mouseleave", () => {
      if (!bullet.classList.contains("swiper-pagination-bullet-active")) {
        svgPath.style.fill = "none"; // Reset fill if not active
      } else {
        svgPath.style.fill = "var(--swatch--brand)"; // Keep fill if active
      }
    });
  });
}

// SVG gradient definition (append to body for gradient use)
const svgGradient = `
