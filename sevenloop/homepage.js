...
// Pagination hover effect to change opacity of fill color
document.addEventListener("mouseover", function (event) {
  if (event.target.closest(".swiper-pagination-bullet")) {
    const svgPath = event.target.closest(".swiper-pagination-bullet").querySelector(".svg-path");
    svgPath.setAttribute("fill", "var(--swatch--brand)");
...
document.addEventListener("mouseout", function (event) {
  if (event.target.closest(".swiper-pagination-bullet")) {
    const svgPath = event.target.closest(".swiper-pagination-bullet").querySelector(".svg-path");
    if (!event.target.closest(".swiper-pagination-bullet").classList.contains("swiper-pagination-bullet-active")) {
      svgPath.removeAttribute("fill");
    }
    svgPath.style.opacity = ""; // Reset opacity
  }
});
